...
class Ucty extends My_Controller
{
	...
	public function zmena_hesla()
	{
		...
		$form->addText( "acc_username" )
				  ->set_value( User::get_username() )
				  ->set_attribute( "class", "span4" )
				  ->set_rule( "nospace", "Uživatelské jméno nesmí obsahovat mezeru." )
				  ->set_rule( "onlychars", "Uživatelské jméno musí obsahovat jenom písmena a číslice" )
				  ->set_rule( Form::RULE_MIN_CHARS, "Uživatelské jméno musí mít alespoň %argument% písmen",5 )
				  ->set_rule( Form::RULE_FILLED, "Uživatelské jméno musí být vyplněné" )
				  ->set_form_attribute( "id", "form-edit" )
				  ->addText( "acc_pass1" )
				  ->set_attribute( "class", "span4" )
				  ->set_attribute( "placeholder", "nové heslo" )
				  ->addText( "acc_pass2" )
				  ->set_attribute( "placeholder", "zopakovat nové heslo" )
				  ->set_rule( Form::RULE_SAME_AS, "Hesla se neshodují", 'acc_pass1' )
				  ->set_attribute( "class", "span4" )
