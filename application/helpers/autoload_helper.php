...
class Autoloader
{
	...
	static function init()
	{
		...
		self::$timer = new timer( true );
		$ci->load->library( 'cache' );
		self::$timer->mark( 'cache loaded' );
		$mapper = new autoloader_mapper( self::$folders );
		self::$timer->mark( 'atlc mapper loaded' );
		//$mapper->createMap();
		self::$timer->mark( 'atlc mapper map created' );
		self::$finder = new autoloader_finder( $mapper );
		self::$timer->mark( 'atlc finder loaded' );
	}
