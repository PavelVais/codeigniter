...
class Minify extends CI_Driver_Library {
	...
	private function _do_combine($files, $type, $compact = TRUE, $css_charset = 'utf-8') {
		...
		{
			$external = \Head\Common_Object::is_url_external( $file );
			if ( !$external && !file_exists( $file ) ) {
				\FB::info( $file, 'soubor nenalezen' );
			...
			$path_info = $external ? $file : pathinfo( $file, PATHINFO_BASENAME ); // Referal File and path

			if ( $external )
				$file = $this->get_external_content( $file );
	 ...
	 * @return bool
	 */
	public function get_file($full_path) {
		$this->_ci->load->helper( 'file' );
