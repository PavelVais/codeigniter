	...
	$.easyconfirm = function(options) {
		...
		return this.each(function() {
			...
			var urlClick = function() {
				...
				if ($target.attr('class') != undefined && $target.attr('class').indexOf("ajax") != -1)
				{
					$.get(target.href);
				} else
				if (target.href) {
			...
			{
				locale.text = $target.data('confirm'); 
			} else 
			...
			var buttons = {};
			buttons[locale.button[1]] = function(event) {
